...
int distributionMosaic(const char* inputPath,
    ...
    try {
        ...
        int nTaille=nH*nW;
        OCTET* ImgIn=nullptr;  allocation_tableau(ImgIn,OCTET,nTaille);
        OCTET* ImgOut=nullptr; allocation_tableau(ImgOut,OCTET,nTaille);
        lire_image_pgm(inputPath,ImgIn,nTaille);

        struct Imag { int ID; std::vector<int> dist; bool used; };
